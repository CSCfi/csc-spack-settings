...
class Tau(Package):
    ...
    def install(self, spec, prefix):
        ...
        # TAUROOT is incorrectly set to the build directory 
        # Here we fix it to point to the install directory 
        TAUROOT_S=r"TAUROOT=[a-z,A-Z,0-9,_,\/,-.]+\s*$"
        TAUROOT_R="TAUROOT="+self.prefix+"\n"
        inc_Makefile=join_path(self.prefix,"include/Makefile")
        CONF_ENV_S=r"CONF_ENV_FILE=[a-z,A-Z,0-9,_,\/,-.]+\s*$"
        CONF_ENV_R="CONF_ENV_FILE="+join_path(self.prefix,".spack/spack-build-env.txt")+"\n"

        filter_file(TAUROOT_S,TAUROOT_R,inc_Makefile)
        filter_file(CONF_ENV_S,CONF_ENV_R,inc_Makefile)
        ...
        filter_file(TAUROOT_S,TAUROOT_R,files[0])
        filter_file(CONF_ENV_S,CONF_ENV_R,files[0])

        # OPENGL causes a very serious crash when enabled
        # Crashes local display server
        # So we disable it
        prof_file=join_path(self.prefix,"bin","paraprof")
        filter_file(r"JAVA2D_OPENGL_FLAG=1","JAVA2D_OPENGL_FLAG=0",prof_file)
    ...
    def setup_run_environment(self, env):
        pattern = join_path(self.prefix.lib, 'Makefile.*')
        files = glob.glob(pattern)
        # Filter file leaves some backups ending in  ~
        files_no_bu=[ f for f in files if f[-1]!="~"]
        ...
        # in the latter case.
        if files_no_bu:    
            env.set('TAU_MAKEFILE', files_no_bu[0])

...
class FluxCore(AutotoolsPackage):
    ...
    version('0.23.1', sha256='04d0cc5d9a527ce0d314cb47a384030bf142d5041103a6e2b6ce9a0ea43c7593')
    ...
    depends_on("lua@5.1:5.4.99", type=('build', 'run', 'link'))
    depends_on("lua@5.1:5.1.99", when="@0.1.0:0.9.0")
    depends_on("lua@5.1:5.2.99", when="@0.10.0:0.19.99")
    depends_on("lua@5.1:5.4.99", when="@0.20.0:,master")
    ...
    depends_on("jansson@2.10:", when="@0.23.0:")
    depends_on("jansson@:2.10", when="@0.22.0:")    
    ...
    def setup_run_environment(self, env):
        ...
        # Add python lib dependencies to the module
        pypath = []
        for dep in ['py-six', 'py-pyyaml', 'py-cffi']:
            pypath.append(os.path.join(
                self.spec[dep].prefix.lib,
                "python{0}".format(self.spec['python'].version.up_to(2)),
                "site-packages"),
            )   
        env.prepend_path('PYTHONPATH', ':'.join(pypath))
